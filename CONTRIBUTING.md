...
1. Add a **minimal** reproduction as a string (or tiny `.fountain` under `Tests/FountainPackageTests/Fixtures/` when shared across tests). If you add a fixture file, append its basename to **`FountainPackageBundledFountainFixtures.basenames`** (sorted list) so **Phase 7** inventory and **fast** vs **tokenPipeline** parity tests stay aligned.
2. Add or extend an SPM test in **`Tests/FountainPackageTests/`** (e.g. `SpecTraceabilityTests`, `Phase5ProductionFeaturesTests`, `Phase7ComplianceTests`) so `swift test` catches the regression. If you touch the tokenizer pipeline, extend **`Phase4ParityTests`** and **`TokenPipelineFNScriptTests`** so **`.fast`** and **`.tokenPipeline`** stay aligned. **`FountainUI`** changes go in **`Tests/FountainUIPackageTests/`**.
