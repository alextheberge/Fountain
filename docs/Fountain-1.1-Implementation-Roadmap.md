...
**Status:** **Complete (initial)** — unified ``FountainScriptRendering`` API; plaintext / Markdown / JSON / HTML + **FDX / PDF** exporters with tests. Legacy ``FountainWriter`` remains for Fountain body/title string export; new call sites should prefer protocol conformers. **Follow-up:** **8.5** / **8.6** improve **cross-platform** pagination measurement and **PDF** portability (Wasm / Linux) — **8.5** ``FNPaginator`` **closure + generic** measurer path landed; **8.6** stub path landed (see Phase 8 table). **8.7** **done (initial):** ``FountainFDXWriter`` emits **`<PageLayout>`**, **`<ElementSettings>`**, and **`<MoresAndContinueds>`** (golden + regression tests). **8.8** **partial:** PDF **page numbers**; **(MORE)** / **(CONT’D)** via ``FNPaginator`` + writer still **open**.
...
| 8.5 | **Abstract text measurement** out of ``FNPaginator`` behind a protocol (e.g. **`TextMeasurer`**) so **core pagination math** does not hard-code **UIKit/AppKit** font metrics. Ship a **default** implementation in **FountainHTML** (or a thin **`FountainApple`** product) using **`NSAttributedString` / `UIFont` / `NSFont`** (or equivalent) on Apple platforms; **Wasm / Linux** consumers inject their own measurer (e.g. **HTML Canvas**-backed layout in JS, or a stub for tests). | **Partial:** ``FNPaginator`` uses a **closure** + ``layoutLineHeight`` (no stored `any` — avoids runtime crash); ``init(script:)`` → ``AppKitFountainTextMeasurer``; ``init(script:textMeasurer:)`` generic over ``FountainTextMeasuring``; ``init(script:layoutLineHeight:measureHeight:)`` for custom backends. Types + ``FountainTextMeasuringTests`` + paginator smoke. **Open:** document host-injected Canvas measurer in [SwiftWasm-Experimental.md](SwiftWasm-Experimental.md); optional **fake** shared across CI matrix. |
| 8.6 | **PDF portability:** if **FountainPDFWriter** stays in-repo, either (a) adopt a **lightweight cross-platform** PDF generator that does not require **CoreGraphics** on every OS, **or** (b) **strictly isolate** ``FountainPDFWriter`` (and any CoreText/CoreGraphics-only code paths) behind **`#if canImport(CoreGraphics)`** (and related guards), matching the existing **wasm32** stub story with a clear **Linux** story. | **Partial:** ``FountainPDFWriter`` real implementation is behind **`#if arch(wasm32) || !canImport(CoreGraphics) || !canImport(CoreText)`** stub vs full PDF; same stub error surface as Wasm when CG/CT unavailable. **Open:** ADR / Linux CI story; **Wasm** script unchanged. |
...
| **`FountainTextMeasuring`** + ``FNPaginator`` injection (non-Apple measurers) | 8.5 / 10 | ``FNPaginator`` + closure injection, ``FountainTextMeasuringTests``; [SwiftWasm-Experimental.md](SwiftWasm-Experimental.md) Canvas measurer note **open** | ☐ |
| **PDF** portable or **CoreGraphics**-isolated (`FountainPDFWriter`) | 8.6 / 10 | ``FountainPDFWriter`` `#if canImport` stub path, [SwiftWasm-Experimental.md](SwiftWasm-Experimental.md) | ☐ |
...
| **PDF** MORE / CONT’D across pages + header page numbers | 8.8 | ``FountainPDFWriter`` page numbers + PDFKit multi-page test; ``FNPaginator`` (MORE) + writer integration **open** | ☐ |
...
| Line → element index map | 9.4 | ``FountainLineToElementIndexMap``, ``FountainLineToElementIndexMapTests`` (MVP: logical body-line → element); full offset / interval APIs TBD | ☐ |
...
7. **Phase 8** — Writers / ``FountainScriptRendering`` (**initial complete**; FDX/PDF baseline shipped — **8.7** **initial** FDX layout metadata shipped; **8.5**/**8.6** cross-platform + PDF portability; **8.8** screenplay PDF pagination).  
...
| **Phase 8** | Deeper HTML/CSS refactor if desired. **Polish:** ``FountainStubRendererError`` retained for future optional stubs; conforms to ``LocalizedError``. **8.5** **started** — ``FountainTextMeasuring`` types; **8.6** **started** — PDF behind **canImport** stub; **8.7** **initial** FDX metadata shipped; **8.8** **partial** — PDF **page numbers**; **(MORE)**/**(CONT’D)** + paginator–writer contract **open** — see Phase 8 table. |
