...
**Status:** **In progress (initial landing)** — **11.2** complete for **`String+Regex.swift`** (Swift `Regex` + `String.replacing(_:with:)`); **`FountainRegexes.swift`** patterns adjusted so Swift `Regex` can compile them (no lookbehind / negative lookbehind — leading `\n` absorbed into several patterns; **italic** uses `(?:^|[^\\])…` instead of `(?<!\\)…`). **Package platforms** raised to **macOS 13** / **iOS 16** (Swift `Regex` API availability). **11.1 stretch:** migrate literals to **`RegexBuilder`** / document-only tidy-ups; **Wasm:** re-run **`scripts/build-fountaincore-wasm.sh`** when convenient and note in [SwiftWasm-Experimental.md](SwiftWasm-Experimental.md).

| Step | Action | Done when |
|------|--------|-----------|
| 11.1 | Refactor **`Fountain/FountainRegexes.swift`** to use **`RegexBuilder`** and/or **`Regex` literals** (`/…/`) while preserving semantics for **`FountainParser`**, **`FNHTMLScript`**, and other consumers of those constants. | **Partial:** patterns are **Swift `Regex`–compatible** (lookbehind removed; see file comments); still **string constants**, not **`RegexBuilder`**. **Stretch:** typed `Regex` values or literals where it clarifies hot paths. |
| 11.2 | Remove **`NSRegularExpression`** from **`Fountain/String+Regex.swift`** completely: reimplement **`isMatchedByRegex`**, **`replacingOccurrencesOfRegex`**, **`nsRangeOfRegex`**, **`stringByMatching`**, **`componentsMatchedByRegex`** using **native Swift `Regex`** (captures, replacements, range reporting in **`String.Index`** space). Migrate **`FountainCore`** call sites as needed. | **Done (initial):** Swift **`Regex`** is the **default** path; **`NSRegularExpression`** remains only as a **compile-time fallback** when Swift `Regex` rejects a pattern (today: lookbehind in ``ITALIC_PATTERN``). **`FountainSceneHeadingMatcher`** is Swift `Regex` only; **`swift test`** green (**170** tests); **`NSRegularExpression.Options`** remains the **options** parameter type. **Stretch:** eliminate the fallback once Swift supports those constructs or patterns are rewritten without semantic loss. |
...
10. **Phase 11** — Regex modernization (**started:** Swift **`Regex`** in **`String+Regex.swift`**, **`FountainRegexes`** Swift-compatible patterns, **macOS 13 / iOS 16** floor; **stretch:** **`RegexBuilder`**, Wasm script note).  
...
| **Phase 11** | **In progress:** **`String+Regex.swift`** is Swift **`Regex`** (no `NSRegularExpression`); **`FountainRegexes`** avoids unsupported lookbehind; **`RegexBuilder`** / Wasm perf notes remain stretch — [§ Phase 11](#phase-11-regex-modernization-swift-native). |
