...
**Phase 1.3 status:** **Complete** for documentation and stability expectations below. **Phase 7** (SwiftPM test matrix and external-suite tracking) is documented in [Fountain-1.1-Implementation-Roadmap.md](Fountain-1.1-Implementation-Roadmap.md) and [External-Fountain-Test-References.md](External-Fountain-Test-References.md). **Phase 9** (async full parse + element streaming; incremental parse explicitly deferred) is summarized in the roadmap and [Fountain-Incremental-Parse-Spike.md](Fountain-Incremental-Parse-Spike.md). **`@_spi(…)`** is not applied in source yet; when a subsystem stabilizes, you may narrow visibility without changing runtime behavior.
