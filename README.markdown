...
The Xcode project **Fountain** targets **macOS 12+** and **iOS 15+**; sample apps and **`FountainTests`** link the **Fountain** Swift package product (core + HTML). **Legacy Objective-C** sources remain in the tree for reference. The library reads and writes Fountain files and stores the script in a generic data model. If this model is insufficient for your needs, or you have your own model you'd like to use, we recommend using a converter to bridge the two models.
