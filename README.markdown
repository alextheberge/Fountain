...
- **Gap analysis:** [docs/Fountain-1.1-Gap-Analysis.md](docs/Fountain-1.1-Gap-Analysis.md) — **Phase 0 baseline complete** (parser inventory, regex classification, feature matrix); file stays updated as later phases land.
- **SwiftPM (started):** `Package.swift` at the repo root defines **`FountainCore`** (parse/model/write, no UI), **`FountainHTML`** (`FNHTMLScript`, `FNPaginator`, `Platform`, CSS resource), and an umbrella **`Fountain`** product that re-exports both (import **`Fountain`** in apps). Objective-C `Legacy/` is excluded. The manifest package name is **`FountainSwiftPM`** so it does not collide with the `Fountain` library target under `swift test`. From the repository root run **`swift build`** and **`swift test`** (macOS 12+ / Swift 5.9+). `FNHTMLScript` loads `ScriptCSS.css` via `Bundle.module` when built as a package. **CI:** pushes and PRs to **`master`** run **`swift build`** and **`swift test`** (`.github/workflows/swift.yml`). Tests include **`Phase5ProductionFeaturesTests`**, **`BrickSteelCorpusTests`**, and corpus smoke tests. **Tooling:** **`FountainJSONWriter`** / **`FNScript.fountainDocumentJSONData`** emit `FountainDocument` JSON; **`FNScript.metrics`** gives word/element counts (plus scene, transition, character-cue, and dialogue-element counts) with boneyard excluded from body-wide totals; **`FountainInlineMarkup.attributedFragment`** builds `AttributedString` for inline bold/italic (see API doc for underline limits).
- **Deprecation stance (lightweight):** the Xcode project continues to compile the same Swift sources as SwiftPM for samples and `FountainTests`; **`Legacy/`** Objective-C remains reference-only and is **not** part of the package. New work should land in **`FountainCore`** / **`FountainHTML`** and stay out of RegexKitLite unless fixing critical bugs. **Details:** [docs/Deprecation-And-Distribution.md](docs/Deprecation-And-Distribution.md).
- **Xcode vs SwiftPM (Phase 1.2):** sample apps still **compile the `Fountain/` sources in the Xcode target** (with `PRODUCT_MODULE_NAME = Fountain` so `FountainTests` can `@testable import Fountain` via the Mac sample host). For **package-only** development, open **`Package.swift`** in Xcode or run **`swift build` / `swift test`**. Wiring the `.xcodeproj` targets to the local package is a follow-up (test host + module layout need a deliberate migration). **Same topic:** [docs/Deprecation-And-Distribution.md](docs/Deprecation-And-Distribution.md).
- **Public API map (Phase 1.3):** [docs/Public-API-Surface.md](docs/Public-API-Surface.md) — modules, stability notes, experimental areas.
- **Other implementations & fixtures (Phase 7.3):** there is no single blessed cross-language test corpus. Useful references: [Fountain syntax](https://fountain.io/syntax/), this repo’s **`FountainTests/`** samples (**Big Fish**, **Brick And Steel**, element fixtures), [nyousefi/Fountain](https://github.com/nyousefi/Fountain) (original Obj‑C + tests), [Fountain.js](https://github.com/mattdaly/Fountain.js) and [fountain-js](https://github.com/jonnygreenwald/fountain-js) (JS parsers with **Brick & Steel**-style checks). Treat licenses separately if you vendor third-party tests.
